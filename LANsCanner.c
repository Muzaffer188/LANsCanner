...
/*
 * Reads a single ARP reply from fd with a timeout.
 * Return 0 on success, or -1 on timeout.
 */
int read_arp(int fd, int timeout_sec)
{
    debug("read_arp with timeout");
    int ret = -1;
    unsigned char buffer[BUF_SIZE];
...
while(1) {
    int r = read_arp(arp_fd, 5);
    if (r == 0) {
        info("Got reply, break out");
        break;
    } else {
        info("No reply for IP, moving to next.");
        break; 
    }
}
    ret = 0;
...
int main(int argc, const char **argv) {
    int ret = -1;
    if (argc != 3) {
        printf("Usage: %s <INTERFACE> <DEST_IP_PREFIX>\n", argv[0]);
        return 1;
    }
    const char *ifname = argv[1];
    const char *ip_prefix = argv[2];
